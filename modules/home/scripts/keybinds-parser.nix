...
pkgs.writeShellScriptBin "keybinds-parser" ''
    ...
    case "$MODE" in
      hyprland)
        # First, try to parse bindd entries from Nix config for richer descriptions
        BIND_NIX="''${KEYBINDS_BIND_NIX:-$HOME/.config/nixos/modules/home/hyprland/binds.nix}"
        if [[ ! -f "$BIND_NIX" && -f "$HOME/zaneyos/modules/home/hyprland/binds.nix" ]]; then
  ...
  BEGIN {
      ...
      sway)
        ...
        if [[ ! -f "$SWAY_CONFIG" ]]; then
          if [[ -f "$HOME/.config/nixos/modules/home/gui/sway/files/config_cuerdos" ]]; then
            SWAY_CONFIG="$HOME/.config/nixos/modules/home/gui/sway/files/config_cuerdos"
          elif [[ -f "$HOME/zaneyos/modules/home/gui/sway/files/config_cuerdos" ]]; then
